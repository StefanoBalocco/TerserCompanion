...
test( 'sort comparator exercises priorityLength, replacedLength, and stable encounter order', ( t: ExecutionContext ) => {
	...
	const constStart1: number = result1.indexOf( 'const ' );
	const declEnd1: number = result1.indexOf( ';\n', constStart1 );
	const decl1: string = result1.slice( constStart1, declEnd1 );
	// Assert each expected initializer occurs exactly once in the declaration
	const aaaaInDecl: number = ( decl1.match( /"aaaa_really_long_profitable_repeated_literal"/g ) || [] ).length;
	t.is( aaaaInDecl, 1, 'aaaa initializer appears exactly once in declaration' );
	const bbbbInDecl: number = ( decl1.match( /"bbbb_really_long_profitable_repeated_literal"/g ) || [] ).length;
	t.is( bbbbInDecl, 1, 'bbbb initializer appears exactly once in declaration' );
	const ccccInDecl: number = ( decl1.match( /"cccc_longer_profitable_repeated_demo"/g ) || [] ).length;
	t.is( ccccInDecl, 1, 'cccc initializer appears exactly once in declaration' );
	const ddddInDecl: number = ( decl1.match( /"dddd_longer_profitable_repeated_demo"/g ) || [] ).length;
	t.is( ddddInDecl, 1, 'dddd initializer appears exactly once in declaration' );
	const eeeeInDecl: number = ( decl1.match( /"eeee_short_profitable_repeated"/g ) || [] ).length;
	t.is( eeeeInDecl, 1, 'eeee initializer appears exactly once in declaration' );
	const ffffInDecl: number = ( decl1.match( /"ffff_short_profitable_repeated"/g ) || [] ).length;
	...
	const aaaaTotal: number = ( result1.match( /"aaaa_really_long_profitable_repeated_literal"/g ) || [] ).length;
	t.is( aaaaTotal, 1, 'aaaa literal appears exactly once in output' );
	const ffffTotal: number = ( result1.match( /"ffff_short_profitable_repeated"/g ) || [] ).length;
...
test( 'CaseClause string literals are aliased with leading separator', ( t: ExecutionContext ) => {
	const source: string = 'switch(e){case"Array":f();break;case"Array":g();break;case"Array":h();break;case"Array":i()}';
	const result: string = TerserCompanion( source );
	...
	const caseAliasCount: number = ( result.match( /\bcase [a-z]:/g ) || [] ).length;
	t.is( caseAliasCount, 4, 'all four clauses must use alias with space after case' );

	const fusedCase: Nullable<RegExpMatchArray> = result.match( /\bcase[a-z]:/g );
	t.falsy( fusedCase, 'no fused case<alias>: token' );

	// Original literal appears only once (in the alias initializer)
	const literalCount: number = ( result.match( /"Array"/g ) || [] ).length;
	...
	const parsed: ts.SourceFile = ts.createSourceFile( 'verify.js', result, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS );
	let caseClauseCount: number = 0;

	function countClauses( node: ts.Node ): void {
...
test( 'typeof left boundary requires separator space', ( t: ExecutionContext ) => {
	const literal: string = 'left_boundary_test_repeated_literal_string_xyz';
	const source: string = [
		'x=typeof"' + literal + '";',
		'y=typeof"' + literal + '";',
		'z=typeof"' + literal + '";',
	...
	const typeOfAliasCount: number = ( result.match( /\btypeof [a-z];/g ) || [] ).length;
	t.is( typeOfAliasCount, 4, 'all four typeof expressions must have space before alias' );

	const fusedTypeof: Nullable<RegExpMatchArray> = result.match( /\btypeof[a-z];/g );
...
test( '"…" in expression right boundary requires separator space', ( t: ExecutionContext ) => {
	const literal: string = 'right_boundary_test_repeated_literal_string_xyz';
	const source: string = [
		'x="' + literal + '"in a;',
		'y="' + literal + '"in b;',
		'z="' + literal + '"in c;',
	...
	const aliasInCount: number = ( result.match( /\b[a-z] in [a-z];/g ) || [] ).length;
	t.is( aliasInCount, 4, 'all four in-expressions must have space between alias and in' );

	const fusedIn: Nullable<RegExpMatchArray> = result.match( /\b[a-z]in [a-z];/g );
...
test( 'mixed contexts apply separators per occurrence', ( t: ExecutionContext ) => {
	const literal: string = 'mixed_context_test_repeated_literal_string_xyz';
	const source: string = [
		'x="' + literal + '";',
		'y="' + literal + '";',
	...
	const literalPattern: RegExp = new RegExp( '"' + literal + '"', 'g' );
	const literalCount: number = ( result.match( literalPattern ) || [] ).length;
	t.is( literalCount, 1, 'original literal must appear exactly once' );
...
test( 'separator cost deduction rejects break-even case', ( t: ExecutionContext ) => {
	...
	const source: string = 'switch(x){case"abcdefghijk":case"abcdefghijk":break}';
...
test( 'exact declaration-cost one-character saving', ( t: ExecutionContext ) => {
	...
	const source: string = 'x="abcdefghij";y="abcdefghij";';
	const result: string = TerserCompanion( source );
...
test( 'direct delete operand remains literal while safe occurrences alias', ( t: ExecutionContext ) => {
	const literal: string = 'a repeated delete literal long enough for alias';
	const source: string = [
		'var a = "' + literal + '";',
		'var b = "' + literal + '";',
		'var c = "' + literal + '";',
	...
	const literalCount: number = ( result.match( new RegExp( '"' + literal + '"', 'g' ) ) || [] ).length;
...
test( 'with statement combined with profitable candidates returns unchanged', ( t: ExecutionContext ) => {
	const source: string = 'with( obj ) {\nvar a = "repeated long profitable string which should remain";\nvar b = "repeated long profitable string which should remain";\n}\n';
	t.is( TerserCompanion( source ), source );
} );

// ── Contract 31: direct eval passthrough ───────────────────────────────────────

test( 'direct eval with profitable candidate returns unchanged', ( t: ExecutionContext ) => {
	const source: string = 'eval( "code" );\nvar a = "a long profitable repeated string that would otherwise be aliased";\nvar b = "a long profitable repeated string that would otherwise be aliased";\n';
	t.is( TerserCompanion( source ), source );
} );

test( 'parenthesized direct eval with profitable candidate returns unchanged', ( t: ExecutionContext ) => {
...
test( 'indirect eval, property eval, optional eval, and Function still transform candidates', ( t: ExecutionContext ) => {
	...
	const tests: { description: string; source: string }[] = [
		{
	...
	const cL1: number = tests.length;

	for( let iL1: number = 0; iL1 < cL1; iL1++ ) {
		const { description, source }: { description: string; source: string } = tests[ iL1 ];
...
test( 'direct require and require.resolve first argument remains literal', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-module-name-for-require-protection-test';
	const source: string = [
	...
	const requirePattern: RegExp = new RegExp( 'require\\( "' + literal + '" \\)', 'g' );
	const requireMatchCount: number = ( result.match( requirePattern ) || [] ).length;
	t.is( requireMatchCount, 2, 'both require( "literal" ) calls retain the literal first argument' );
	assertValidOutput( t, result );
} );

test( 'require.resolve first argument remains literal while safe occurrences alias', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-resolve-target-for-require-dot-resolve-test';
	const source: string = [
	...
	t.true( result.length < source.length, 'output shorter (safe occurrences aliased)' );
	const resolvePattern: RegExp = new RegExp( 'require\\.resolve\\( "' + literal + '" \\)', 'g' );
	const resolveMatchCount: number = ( result.match( resolvePattern ) || [] ).length;
	t.is( resolveMatchCount, 2, 'both require.resolve( "literal" ) calls retain the literal first argument' );
	assertValidOutput( t, result );
} );

test( 'indirect require is not protected, same string aliases', ( t: ExecutionContext ) => {
	const literal: string = 'a-string-that-appears-in-both-indirect-require-and-safe-contexts';
	const source: string = [
		'(0,require)( "' + literal + '" );',
		'(0,require)( "' + literal + '" );',
		'var safe = "' + literal + '";',
...
test( 'independent same-root function candidates', ( t: ExecutionContext ) => {
	const source: string = [
	...
	t.true( result.length < source.length, 'output shorter' );
	const floorInits: number = ( result.match( /\bMath\.floor\b/g ) || [] ).length;
	t.is( floorInits, 1, 'Math.floor appears only in the declaration initializer' );
	const roundInits: number = ( result.match( /\bMath\.round\b/g ) || [] ).length;
...
test( 'class root takes precedence over overlapping function path', ( t: ExecutionContext ) => {
	const source: string = [
	...
	const arrayFromInit: Nullable<RegExpMatchArray> = result.match( /\bconst\s+[a-z]+=Array\.from\b/ );
...
test( 'reverse-alphabetical encounter-order tiebreak replaces localeCompare', ( t: ExecutionContext ) => {
	...
	t.true( result.length < source.length, 'output shorter' );
	// Extract the top-level alias declaration
	const constStartR: number = result.indexOf( 'const ' );
	const declEndR: number = result.indexOf( ';\n', constStartR );
	const declR: string = result.slice( constStartR, declEndR );
	// Assert each initializer occurs exactly once in the declaration
	const zzzzInDecl: number = ( declR.match( /"zzzz_really_really_long_profitable_repeated"/g ) || [] ).length;
	t.is( zzzzInDecl, 1, 'zzzz initializer appears exactly once in declaration' );
	const aaaaInDeclR: number = ( declR.match( /"aaaa_really_really_long_profitable_repeated"/g ) || [] ).length;
	...
	const zzzzTotal: number = ( result.match( /"zzzz_really_really_long_profitable_repeated"/g ) || [] ).length;
	t.is( zzzzTotal, 1, 'zzzz literal appears exactly once in output' );
	const aaaaTotalR: number = ( result.match( /"aaaa_really_really_long_profitable_repeated"/g ) || [] ).length;
...
test( 'plain source: first Math.floor starts at insertion point 0', ( t: ExecutionContext ) => {
	const source: string = 'Math.floor(1.5);var b=Math.floor(2.5);var c=Math.floor(3.5);var d=Math.floor(4.5);';
	const result: string = TerserCompanion( source );
...
test( 'directive followed immediately by Math.floor calls', ( t: ExecutionContext ) => {
	const source: string = '"use strict";Math.floor(1.5);var b=Math.floor(2.5);var c=Math.floor(3.5);var d=Math.floor(4.5);';
	const result: string = TerserCompanion( source );
...
test( 'import declaration followed immediately by Math.floor calls', ( t: ExecutionContext ) => {
	const source: string = 'import x from"y";Math.floor(1.5);var b=Math.floor(2.5);var c=Math.floor(3.5);var d=Math.floor(4.5);';
	const result: string = TerserCompanion( source );
...
test( 'shebang newline followed immediately by Math.floor calls', ( t: ExecutionContext ) => {
...
test( 'require first argument literal is preserved verbatim', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-module-name-for-require-protection-test';
	const source: string = [
		'var mod1 = require( "' + literal + '" );',
		'var mod2 = require( "' + literal + '" );',
...
test( 'require.resolve first argument literal is preserved verbatim', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-resolve-target-for-require-dot-resolve-test';
	const source: string = [
		'var path1 = require.resolve( "' + literal + '" );',
		'var path2 = require.resolve( "' + literal + '" );',
...
test( 'non-direct require forms still alias the first string argument', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-repeated-literal-for-non-direct-require-test';
	const tests: { description: string; source: string }[] = [
		{
...
test( 'non-direct require.resolve forms still alias the first string argument', ( t: ExecutionContext ) => {
	const literal: string = 'a-very-long-repeated-literal-for-non-direct-resolve-test';
	const tests: { description: string; source: string }[] = [
		{
...
test( 'config-order tiebreak for exact functions with equal lengths and counts', ( t: ExecutionContext ) => {
	const source: string = [
	...
	t.true( result.length < source.length, 'output shorter' );
	// Extract the top-level alias declaration
	const constStartF: number = result.indexOf( 'const ' );
	const declEndF: number = result.indexOf( ';\n', constStartF );
	const declF: string = result.slice( constStartF, declEndF );
	// Assert each expected initializer occurs exactly once in the declaration
	const zuluInDecl: number = ( declF.match( /\bZulu\.run\b/g ) || [] ).length;
	t.is( zuluInDecl, 1, 'Zulu.run initializer appears exactly once in declaration' );
	const alfaInDecl: number = ( declF.match( /\bAlfa\.run\b/g ) || [] ).length;
	...
	const zuluTotal: number = ( result.match( /\bZulu\.run\b/g ) || [] ).length;
	t.is( zuluTotal, 1, 'Zulu.run appears exactly once in output' );
	const alfaTotal: number = ( result.match( /\bAlfa\.run\b/g ) || [] ).length;
...
test( 'config-order tiebreak for class roots with equal lengths and counts', ( t: ExecutionContext ) => {
	const source: string = [
	...
	t.true( result.length < source.length, 'output shorter' );
	// Extract the top-level alias declaration
	const constStartC: number = result.indexOf( 'const ' );
	const declEndC: number = result.indexOf( ';\n', constStartC );
	const declC: string = result.slice( constStartC, declEndC );
	// Assert each expected initializer occurs exactly once in the declaration
	const zuluRootInDecl: number = ( declC.match( /\bZuluRoot\b/g ) || [] ).length;
	t.is( zuluRootInDecl, 1, 'ZuluRoot initializer appears exactly once in declaration' );
	const alfaRootInDecl: number = ( declC.match( /\bAlfaRoot\b/g ) || [] ).length;
	t.is( alfaRootInDecl, 1, 'AlfaRoot initializer appears exactly once in declaration' );
	// Assert each original initializer appears exactly once in full output (declaration only)
	const zuluRootTotal: number = ( result.match( /\bZuluRoot\b/g ) || [] ).length;
	t.is( zuluRootTotal, 1, 'ZuluRoot appears exactly once in output' );
	const alfaRootTotal: number = ( result.match( /\bAlfaRoot\b/g ) || [] ).length;
